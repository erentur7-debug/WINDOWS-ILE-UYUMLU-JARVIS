...
import os, time, math, random, signal, threading, ctypes, webbrowser
...
class _MciPlayer:
    ...
    def __init__(self, path: Path, volume: float):
        ...
        self.alias = f"jarvis_sfx_{int(time.time() * 1000)}_{type(self)._counter}"
        self._closed = False
        self.returncode = None
        path_str = str(path).replace('"', "")
        _mci_send(f'open "{path_str}" type mpegvideo alias {self.alias}')
        try:
            _mci_send(f"setaudio {self.alias} volume to {int(max(0.0, min(1.0, volume)) * 1000)}")
        except Exception:
            pass
    ...
    def wait(self, timeout: float | None = None):
        ...
        while self.poll() is None:
            if timeout is not None and time.time() - start >= timeout:
                raise TimeoutError("MCI playback wait timed out")
            time.sleep(0.05)
        return self.returncode
...
class SoundManager:
    ...
    def _start_sound(self, path: Path, volume: float):
        proc = _MciPlayer(path, volume)
        with self._lock:
    ...
    def _loop_ambient(self, stop_event: threading.Event):
        while not stop_event.is_set():
            with self._lock:
                ...
                volume = self._volume
            try:
                proc = self._start_sound(_HUD_FILE, volume)
            except Exception:
                break
    ...
    def _foreground_worker(
        ...
        while not stop_event.is_set():
            try:
                proc = self._start_sound(path, volume)
            except Exception:
                break
...
class JarvisUI:
    def __init__(self):
        ...
        # Windows bazen ilk geometry degerini gec uygular, tekrar zorla.
